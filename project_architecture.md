...
- **Hành vi chính:**
  ...
  - Giám sát số liệu tổng quan toàn sàn: số lượng User đăng ký, số Chủ trọ đang hoạt động và tổng số khu trọ/phòng trọ trên hệ thống.
  - Quản lý & xử lý vi phạm: Khóa/mở khóa tài khoản người dùng hoặc chủ trọ nếu có dấu hiệu gian lận, spam, lừa đảo.

#### 🏠 2. LANDLORD – Chủ trọ

- **Vai trò:** Người kinh doanh cho thuê phòng. Chủ trọ đăng ký tài khoản và có thể sử dụng hệ thống sau khi hoàn tất thông tin tài khoản.
- **Hành trình trải nghiệm:**
  1. **Đăng ký & Đăng nhập:** Điền thông tin cá nhân (Họ tên, SĐT, Email, CCCD) → Tài khoản được tạo với trạng thái `ACTIVE` → Đăng nhập và sử dụng hệ thống.
...
## 🎯 PHÂN CÔNG THEO GÓI CHỨC NĂNG

Mỗi người làm trọn một gói từ giao diện đến backend. Khi hoàn thành một gói, nhóm có thể chạy và demo được chức năng đó, không chia nhỏ theo kiểu một người chỉ làm vài file.

### 🔵 HẢO — Gói nền tảng, đăng bài ở ghép và chống tranh chỗ

**Mục tiêu:** Hệ thống có thể chạy được và xử lý an toàn trường hợp nhiều người cùng xin vào một phòng.

**Hảo làm giao diện:**

- Trang danh sách bài đăng tìm người ở ghép.
- Form chủ phòng tạo bài đăng ở ghép.
- Trang xem chi tiết bài đăng.
- Modal để người dùng gửi lời giới thiệu và xin tham gia.
- Trang chủ trọ xem và duyệt yêu cầu ở ghép.

**Hảo làm backend:**

- Eureka Server để các service tìm thấy nhau.
- API Gateway làm cổng vào chung của hệ thống.
- JWT Filter ở Gateway: kiểm tra token, lấy `userId` và `role`.
- API tạo, xem, sửa và đóng bài đăng ở ghép.
- API gửi, xem, duyệt và từ chối yêu cầu ở ghép.
- Redis Lock khi duyệt người vào phòng.
- Kiểm tra sức chứa phòng và cập nhật số người đang ở.
- Tạo bản ghi người thuê khi duyệt thành công.
- Gửi event `join.approved` cho Notification Service.

**Hảo làm hạ tầng và kiểm thử:**

- Docker Compose cho MySQL, Redis, RabbitMQ và các service.
- Cấu hình Redis Redisson.
- Script gửi nhiều request duyệt cùng lúc để chứng minh không bị vượt sức chứa.
...
- Trước khi ghép code, mỗi người phải báo rõ port, endpoint, request body và response mẫu.

---

## 📋 IV. CHECKLIST TICKET CHI TIẾT THEO TỪNG THÀNH VIÊN

_(Mỗi dòng tương ứng 1 file = 1 lần nhờ AI vibe code / Cursor / Claude Code)_
...
- [ ] `service/AuthService.java` (method `registerLandlord`) — Prompt: "Method registerLandlord tương tự register nhưng role=LANDLORD, status=ACTIVE, lưu idCardNumber."
...
- [ ] `service/AdminService.java` (method `getStats`) — Prompt: "Method getStats(): đếm totalUsers (role=USER), totalLandlords (role=LANDLORD) từ UserRepository, gọi PropertyRentalClient lấy totalProperties/totalRooms, trả DTO tổng hợp."
...
#### Task 2.7 — Notification Service & Component Chuông Báo (Ngày 41–48)
